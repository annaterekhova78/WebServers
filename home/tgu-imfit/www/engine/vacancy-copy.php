...
if ($_SESSION['group_user']==2) {
	...
	$menu1="<a class='blue' href='bacalavr_study.php?id_section=1&n=1'>РЕДАКТИРОВАНИЕ КОНТЕНТА</a> &#9672; <a class='blue' href='users.php'>ПОЛЬЗОВАТЕЛИ</a> &#9672; <a class='blue' href='answer.php?id_question=1&n=1'>ОТВЕТЫ НА ВОПРОСЫ</a> &#9672; ";
	}  
elseif(($_SESSION['group_user']==0)){
	$status="представитель компании";$menu1="";
	}
	
echo "<div class='col-12 text-center main-menu pb-15 pt-15'> <a class='red' href=''>РАЗМЕЩЕНИЕ ВАКАНСИЙ</a> &#9672; ".$menu1." </div>";

echo "<div class='col-5 pt-15'><h3 class='red'>Размещение вакансий</h3><a href='../vacancy.html' target='_blank'>Перейти на страницу &laquo;ТРУДОУСТРОЙСТВО&raquo;</a>
</div>
<div class='col-7 pt-15'>На сайте находится ".$status." ".$_SESSION['surname']." ".$_SESSION['name']." ".$_SESSION['secondname'].", &nbsp;<a href='?id_company=".$id_cm."#edit_company' class='blue' title='Редактировать' >".$cm."</a><span class='f-right'><a href='?exit'>Выход  <i class='ti-arrow-right'></i></a></span>
</div>";
 ?>
		<div class="col-12">
...
<p>Требования<br><textarea name='req' id='req'  class="minussize" ></textarea>
<script>
   CKEDITOR.replace("req", {height: 500});
...
<?php 
if (($_SESSION['group_user']==0) || ($_SESSION['group_user']==1)){$w=" AND bs_company.id_company='$id_cm'";} elseif ($_SESSION['group_user']==2) {$w="";}
include "bd.php";
...
echo'<form action="accsess_pub.php" method="POST">
...
<div class="col-xl-9 col-lg-9 col-md-6 col-sm-12 minussize">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="copyright text-center">
                                <p>Copyright © <a href="#">Бизнес-информатика и системный анализ</a></p>
                            </div>
                        </div>
                    </div>
                </div>
...
$y="SELECT * FROM bs_company WHERE id_company='$id_company'";
$y_ex=mysql_query($y);
...
<tr><td>Адрес</td><td><input type="text" name="address" id="address" value="<?php echo $y_res['address']; ?>"></td></tr>
...
<tr><td>Отчество</td><td><input type="text" name="secondname" id="secondname" value="<?php echo $y_res['secondname']; ?>"></td></tr>
	 ...
	 <p align="center" class="mt-15"><input type="submit" value="СОХРАНИТЬ" class="publish text-center" onClick="edit_company()" ><p>
	 <div id="com"></div>
     <a class="close" href="#close"></a>
</div>	 
<script>
...
<script>
function getch(){/*разрешение/запрет публикации*/
	$("#chacc").hide();
	$(".popup").hide();
